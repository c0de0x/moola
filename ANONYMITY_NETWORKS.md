# Anonymity Networks with Moola

Currently only Tor and I2P have been integrated into Moola. The usage of
...
is made or until `moolad` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `moola-wallet-cli` and
`moola-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `moolad` the ip/port of a socks proxy provided by a
...
which tells `moolad` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `moolad` the inbound address, network
...
which tells `moolad` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `moolad`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `moolad` localhost port 30000.
...
`moolad` RPC port - which is independent from the configuration for incoming
...
```
HiddenServiceDir /var/lib/tor/data/moola
...
This will store key information in `/var/lib/tor/data/moola` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/moola/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Moola network should have
similar timestamps.
...
### Intermittent Moola Syncing

If a user only runs `moolad` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `moolad` as often as possible to conceal when transactions are being sent.
...
`moolad` currently selects two outgoing connections every 5 minutes for
...
`moolad` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `moolad` should likely disconnect
