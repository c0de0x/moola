...
Core tests take longer than any other Moola tests, due to the high amount of computational work involved in validating core components.
...
To run only Moola's core tests (after building):
...
To run only Moola's crypto tests (after building):
...
```bash
moolad --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `moola-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Moola's hash tests (after building):
...
To run only Moola's performance tests (after building):
...
To run only Moola's unit tests (after building):
