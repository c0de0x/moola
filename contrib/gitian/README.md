...
*Setup instructions for a Gitian build of Moola.*

Gitian is the deterministic build process that is used to build the Moola CLI
...
```bash
cp moola/contrib/gitian/gitian-build.py .
```
...
fork the [gitian.sigs repository](https://github.com/moola-project/gitian.sigs) and clone it on your host machine, 
or pass the signed assert file back to your build machine.

```bash
git clone git@github.com:moola-project/gitian.sigs.git
...
You can also look in the [gitian.sigs](https://github.com/moola-project/gitian.sigs/) repo and / or [getmoola.org release checksums](https://web.getmoola.org/downloads/hashes.txt) to see if others got the same checksum for the same version tag.  If there is ever a mismatch -- **STOP! Something is wrong**.  Contact others on IRC / github to figure out what is going on.
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/moola-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/moola-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/moola-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/moola-android-*-build.assert
```
...
[moola-project/gitian.sigs](https://github.com/moola-project/gitian.sigs/) repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the [gitian.sigs/gitian-pubkeys/](https://github.com/moola-project/gitian.sigs/tree/master/gitian-pubkeys) directory in a pull request.
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/moolamooo-moola/moola -b moolamooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
